...
gp_save_matrix('data/nfchoa.dat',x,y,p);
% --- WFS
...
gp_save_matrix('data/wfs.dat',x,y,p);
...
gp_save_matrix('data/nfchoa_nls64.dat',x,y,p);
% --- NFCHOA band limited
conf.nfchoa.order = [];
d = driving_function_imp_nfchoa(x0,xs,src,conf);
d = convolution(d,pulse);
[p,x,y] = sound_field_imp(X,Y,Z,x0,'ps',d,t,conf);
gp_save_matrix('data/nfchoa_nls64_band_limited.dat',x,y,p);
% --- WFS
x0 = secondary_source_selection(x0,xs,src);
x0 = secondary_source_tapering(x0,conf);
d = driving_function_imp_wfs(x0,xs,src,conf);
...
gp_save_matrix('data/wfs_nls64.dat',x,y,p);
