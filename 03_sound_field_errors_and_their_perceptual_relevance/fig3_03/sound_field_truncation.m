...
gp_save_loudspeakers('array_truncated.txt',x0);
[P,x,y] = sound_field_mono_wfs(X,Y,Z,xs,src,f,conf);
gp_save_matrix('sound_field_truncated.dat',x,y,db(abs(P)));
...
conf.secondary_sources.x0 = x0;
gp_save_loudspeakers('array_truncated_round.txt',x0);
[P,x,y] = sound_field_mono_wfs(X,Y,Z,xs,src,f,conf);
gp_save_matrix('sound_field_truncated_round.dat',x,y,db(abs(P)));
...
x0 = secondary_source_positions(conf);
conf.secondary_sources.x0 = x0;
gp_save_loudspeakers('array_inf.txt',x0);
[P,x,y] = sound_field_mono_wfs(X,Y,Z,xs,src,f,conf);
gp_save_matrix('sound_field_inf.dat',x,y,db(abs(P)));
