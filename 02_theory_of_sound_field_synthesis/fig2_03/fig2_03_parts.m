...
gp_save_matrix('data/25d_yz.dat',y,z,abs(p));
[p,x,y,z] = sound_field_mono([-2 2],[-1.5 0.5],0,x0,'ps',ones(nls,1),f,conf);
gp_save_matrix('data/25d_xy.dat',x,y,abs(p));

% === 2D/3D sound field ===
p = ones(300);
gp_save_matrix('data/2d_3d_yz.dat',y,x,abs(p));
gp_save_matrix('data/2d_3d_xy.dat',x,y,abs(p));
