...
# signals l(t) and r(t) are the input to the model. After that identical
# monaural preprocessing is applied. The binaural processor calculates the ITD,
# ILD, and interaural vector strength (IVS) based on the arriving input signals
# for every frequency channel. At the end the binaural parameter is mapped to a
# single direction estimation averaged over the whole time of the input signals.
#
...
set terminal epslatex size 12.4cm,10cm color colortext @small
set output 'fig6_01.tex'
...
set label '\acs{ITD}' at 20,117.5 center front tc ls 12                                             #
set label '\acs{ILD}' at 50,117.5 center front tc ls 12                                             #
...
# -- unwrap ITD -----------------------------------------------------------------------------------
set label '\ft \acs{ITD}-$\phi$-mapping' at 0,142.5 right front tc ls 12                            #
set label '\ft unwrap \acs{ITD}' at 20,142.5 center front tc ls 12                                  #
...
plot 'modulationfilterbank.txt' u 1:7 w l ls 11, \
     ''                         u 1:26 w l ls 13
set origin COL2,0.475
replot
unset object

# plot compression
set size 0.04,0.04
set origin COL1+0.019,0.355
comp(x) = x**.4
set xrange [-0.02:1]
set yrange [-0.0:1]
unset logscale
...
unset object
unset label
set origin 0.55,0.74
set ylabel '\tiny $\phi$' offset 2,0 tc ls 12
set xlabel '\tiny \acs{ITD}' offset 0,1.6 tc ls 12
set yrange [-90:90]
set xrange [-0.9:0.9]
load 'greens.pal'
set palette gray
