...
if show_figures
    ...
    for ii=1:size(files_ps,1)
        if Y(ii)~=0 && X(ii)==-1.25 % handle different length of X for different Y values
            ...
            title(sprintf('WFS pw 14 loudspeaker, X = (%1.2f,%1.2f)m',X(ii),Y(ii)));
            figure; hist(phi(ii,2,:,:),-150:10:150)
            title(sprintf('WFS pw 28 loudspeaker, X = (%1.2f,%1.2f)m',X(ii),Y(ii)));
            figure; hist(phi(ii,3,:,:),-150:10:150)
            title(sprintf('WFS pw 56 loudspeaker, X = (%1.2f,%1.2f)m',X(ii),Y(ii)));
        end
    end
end
