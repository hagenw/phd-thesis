...
for jj=1:size(files_ps,2)
    for ii=1:size(files_ps,1)
        ...
        for vp=1:11
            ...
            for nn=1:5
                phi(ii,jj,vp,nn) = ...
                    wierstorf2013estimateazimuth(auralize_ir(ir(:,vp*2-1:vp*2),noise_sig(:,nn),1,conf),lookup) + ...
...
if show_figures
    ...
    for ii=1:size(files_ps,1)
        if Y(ii)~=0 && X(ii)==-1.25 % handle different length of X for different Y values
            ...
            title(sprintf('NFC-HOA ps 14 loudspeaker, 7th order, X = (%1.2f,%1.2f)m',X(ii),Y(ii)));
            figure; hist(phi(ii,2,:,:),-150:10:150)
            title(sprintf('NFC-HOA ps 28 loudspeaker, 14th order, X = (%1.2f,%1.2f)m',X(ii),Y(ii)));
            figure; hist(phi(ii,3,:,:),-150:10:150)
            title(sprintf('NFC-HOA ps 56 loudspeaker, 28th order, X = (%1.2f,%1.2f)m',X(ii),Y(ii)));
            figure; hist(phi(ii,4,:,:),-150:10:150)
            title(sprintf('NFC-HOA ps 14 loudspeaker, 28th order, X = (%1.2f,%1.2f)m',X(ii),Y(ii)));
        end
    end
end
...
get_two_directions(phi,X,Y,15,1,iterations,phi_real);
fprintf('NFC-HOA point source 14 loudspeaker, 7th order\n');
get_two_directions(phi,X,Y,11,1,iterations,phi_real);
fprintf('NFC-HOA point source 14 loudspeaker, 28th order\n');
get_two_directions(phi,X,Y,4,4,iterations,phi_real);
fprintf('NFC-HOA point source 14 loudspeaker, 28th order\n');
get_two_directions(phi,X,Y,2,4,iterations,phi_real);
