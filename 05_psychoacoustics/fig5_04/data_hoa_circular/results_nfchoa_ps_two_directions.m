...
for nn=1:size(files_ps,1)
    for yy=1:length(Y)
        [tmp_data14, ...
         ...
         tmp_real56, ...
         tmp_data1456, ...
         tmp_real1456] = get_pooled_data(files_ps{nn,yy},nconditions(yy));
        % store in cell
    ...
    for yy=1:length(Y)
        for xx=1:length(X)
            if Y(yy)~=0 && X(xx)==-1.25 % handle different length of X for different Y values
                ...
                title(sprintf('NFC-HOA ps 14 loudspeaker, 7th order, X = (%1.2f,%1.2f)m',X(xx),Y(yy)));
                figure; hist(sfs28{yy}(:,xx),-150:10:150)
                title(sprintf('NFC-HOA ps 28 loudspeaker, 14th order, X = (%1.2f,%1.2f)m',X(xx),Y(yy)));
                figure; hist(sfs56{yy}(:,xx),-150:10:150)
                title(sprintf('NFC-HOA ps 56 loudspeaker, 28th order, X = (%1.2f,%1.2f)m',X(xx),Y(yy)));
                figure; hist(sfs1456{yy}(:,xx),-150:10:150)
                title(sprintf('NFC-HOA ps 14 loudspeaker, 28th order, X = (%1.2f,%1.2f)m',X(xx),Y(yy)));
            end
        end
    end
end
...
% calculate the labels for Fig 5.5:
label = [];
for ii=1:iterations
    label(ii,:) = emgm(sfs14{1}(:,5)',2);
