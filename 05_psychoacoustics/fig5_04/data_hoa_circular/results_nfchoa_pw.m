...
for nn=1:size(files_pw,1)
    for yy=1:length(Y)
        [tmp_sfs14_mean, ...
         ...
         tmp_speaker_mean, ...
         tmp_speaker_std, ...
         tmp_speaker_real] = get_single_data(files_pw{nn,yy},nconditions(yy));
        % store in cell
...
std_y1 = mean([tmp14(1:3,:); tmp28(1:4,:); tmp56(1:5,:); tmp1456(1:5,:)],1);
std_y2 = mean([tmp14(6:9,:); tmp28(6:10,:); tmp56(6:11,:); tmp1456(6:11,:)],1);
std_y3 = mean([tmp14(12:16,:); tmp28(12:16,:); tmp56(12:16,:); tmp1456(12:16,:)],1);
std_dev = nanmean(tmp,1);
file = '../std_hoa_circular_pw.txt';
fid = fopen(file,'w');
...
fclose(fid);
fid = fopen('../hoa_nls28_order14_pw.txt','w');
fprintf(fid,'# X/m   Y/m  phi/deg phi_error/deg phi_ci/deg std/deg std_ci/deg\n');
for yy=1:length(Y)
    for xx=1:length(sfs28_phi{:,yy})
        phi = sfs28_phi{:,yy}(xx)+direction_pw(xx,yy);
...
fclose(fid);
fid = fopen('../hoa_nls56_order28_pw.txt','w');
fprintf(fid,'# X/m   Y/m  phi/deg phi_error/deg phi_ci/deg std/deg std_ci/deg\n');
for yy=1:length(Y)
    for xx=1:length(sfs56_phi{:,yy})
        phi = sfs56_phi{:,yy}(xx)+direction_pw(xx,yy);
...
fclose(fid);
fid = fopen('../hoa_nls14_order28_pw.txt','w');
fprintf(fid,'# X/m   Y/m  phi/deg phi_error/deg phi_ci/deg std/deg std_ci/deg\n');
for yy=1:length(Y)
    for xx=1:length(sfs1456_phi{:,yy})
        phi = sfs1456_phi{:,yy}(xx)+direction_pw(xx,yy);
