...
for nn=1:size(files_ps,1)
    for yy=1:length(Y)
        [tmp_sfs14_mean, ...
         ...
         tmp_sfs1456_std, ...
         tmp_speaker_mean, ...
         tmp_speaker_std] = get_single_data(files_ps{nn,yy},nconditions(yy));
        % store in cell
...
% store the results
fid = fopen('../hoa_number_of_sources_nls14_order7_ps.txt','w');
...
fclose(fid);
fid = fopen('../hoa_number_of_sources_nls28_order14_ps.txt','w');
fprintf(fid,'# X/m   Y/m  number_of_sources   number_of_sources_ci\n');
for yy=1:length(Y)
...
fclose(fid);
fid = fopen('../hoa_number_of_sources_nls56_order28_ps.txt','w');
fprintf(fid,'# X/m   Y/m  number_of_sources   number_of_sources_ci\n');
for yy=1:length(Y)
...
fclose(fid);
fid = fopen('../hoa_number_of_sources_nls14_order28_ps.txt','w');
fprintf(fid,'# X/m   Y/m  number_of_sources   number_of_sources_ci\n');
for yy=1:length(Y)
