...
for nn=1:size(files_pw,1)
    for yy=1:length(Y)
        [tmp_data14, ...
         ...
         tmp_real28, ...
         tmp_data56, ...
         tmp_real56] = get_pooled_data(files_pw{nn,yy},nconditions(yy));
        % store in cell
    ...
    for yy=1:length(Y)
        for xx=1:length(X)
            if Y(yy)~=0 && X(xx)==-1.25 % handle different length of X for different Y values
                ...
                title(sprintf('WFS pw 14 loudspeaker, X = (%1.2f,%1.2f)m',X(xx),Y(yy)));
                figure; hist(sfs28{yy}(:,xx),-150:10:150)
                title(sprintf('WFS pw 28 loudspeaker, X = (%1.2f,%1.2f)m',X(xx),Y(yy)));
                figure; hist(sfs56{yy}(:,xx),-150:10:150)
                title(sprintf('WFS pw 56 loudspeaker, X = (%1.2f,%1.2f)m',X(xx),Y(yy)));
            end
        end
    end
end
