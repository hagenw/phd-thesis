...
fclose(fid);
fid = fopen('../wfs_nls28_circular_pw.txt','w');
fprintf(fid,'# X/m   Y/m  phi/deg phi_error/deg phi_ci/deg std/deg std_ci/deg\n');
for yy=1:length(Y)
    for xx=1:length(sfs28_average{:,yy})
        phi = sfs28_average{:,yy}(xx)+direction_pw(xx,yy);
...
fclose(fid);
fid = fopen('../wfs_nls56_circular_pw.txt','w');
fprintf(fid,'# X/m   Y/m  phi/deg phi_error/deg phi_ci/deg std/deg std_ci/deg\n');
for yy=1:length(Y)
    for xx=1:length(sfs56_average{:,yy})
        phi = sfs56_average{:,yy}(xx)+direction_pw(xx,yy);
