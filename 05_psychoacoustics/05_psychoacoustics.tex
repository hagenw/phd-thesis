...
The test participants were placed at 16 different positions: $(0,0.75,0)$\,m,
...
\end{figure}
%
\newthought{In order to} investigate the influence of more than one auditory event on the
...
was found.
%It can be seen that the positions were more than one distribution
%were found correspond not completely with the ones were two perceived auditory
%events were found in Exp.\,4. Four positions lead to the perception of two
%sources in Exp.\,4, but to only one normal distribution in Exp.\,3. And two
%positions had two distributions in Exp.\,3 and only one auditory event in
%Exp.\,4.

\newthought{For the sound field synthesis} method of \ac{WFS} the localization of focused
...
\newthought{For the special case} of the synthesis of a focused source with \ac{WFS} the
...
%One of the reasons why the explicit statement of spatial attributes
%was excluded before is maybe the fact that most of
%the earlier investigations of timbre were all done with headphones.\autocite[Compare for
%example][]{McAdams1995}
%In addition most of them dealed with what is called \emph{musical timbre} and
%worked with sounds from real or synthesized musical instruments.
%For musical timbre they tried to investiagte the number of underlying dimensions
%by applying multidimensional scaling techniques.
%In most cases three independent dimensions were found \TODO{(Zitat)}. But the
%exact attributes of the dimensions depend on the applied stimuli. As well as the
%physical features that influenced these dimensions. For example
%Miller and Carette\autocite{Miller1975} found that the timbre perception was
%dominated by spectral cues in the stimuli whereas
%others\autocite[See for example][]{Iverson1993} found a larger influence of
%temporal features of the sound event.
...
%One problem of binaural decoloration is that the  perception of the
%room as an independent auditory event is suppressed. This includes that the room
%cannot be perceived as colored itself.
%This could be a disadvantage in the case of \ac{SFS} due to the additional wave
%fronts that have a similar time pattern than room reflections.
...
movements.
%These occurred most probably
%because of the non-individual \acp{HRTF} applied for the binaural synthesis.
...
do so.
%For the off-center listening position and stereophony this statement might be too
%strong. The coloration was rated surprisingly high for this condition, but
%further inspection revealed that this position deviated also in other aspects
%from all the \ac{WFS} off-center conditions. Due to the precedence effect the
%source is perceived to come from a different direction. In
%addition, an inspection of the actual root mean square value of the presented
%signals revealed that the stereophonic condition was the loudest of all
%conditions which could be easily confirmed by informal listening.
%It is therefore highly probable that the listeners have included these cues in
%their coloration rating even they were advised not to do so.
...
manikin\sidenote[][-1.3cm]{\cite{Lindau2006}} measured in the anechoic chamber of Technical
